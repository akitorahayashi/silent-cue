...
*   **ターゲット設定:**
    ...
    *   テストターゲットからは、`@testable import SilentCue_Watch_App` を使用してアクセスしますこれにより、プロトコル定義が一元管理されます
...
当初はシングルトンパターンや直接的なAPI呼び出しが多く存在しましたが、TCAの依存性注入システムを活用するようにリファクタリングされました主な変更点は以下の通りです

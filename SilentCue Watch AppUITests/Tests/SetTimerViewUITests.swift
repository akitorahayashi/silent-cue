...
final class SetTimerViewUITests: XCTestCase {
    ...
    func testInitialViewState() throws {
        ...
        XCTAssertFalse(
            app.pickers[SCAccessibilityIdentifiers.SetTimerView.hourMinutePickerView.rawValue]
                .exists
        ) // 初期状態では非表示
    ...
    func testMinutesPickerInteraction() throws {
        ...
        // ピッカーが存在することを確認
        XCTAssertTrue(
            minutesPicker?.waitForExistence(timeout: UITestConstants.Timeout.standard) ?? false,
            "分数ピッカーが存在すること"
        )
